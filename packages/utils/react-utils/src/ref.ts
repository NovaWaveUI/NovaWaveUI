...
export function assignRef<T>(ref: React.Ref<T> | undefined, value: T): void {
	...
	if (typeof ref === 'function') {
		...
		try {
			...
			throw new Error(
				`Cannot assign value "${String(value) || 'unknown'}" to ref "${typeof ref === 'object' ? 'object' : 'unknown'}"`
			);
		}
	}
}
...
export function useDOMRef<T extends Element = HTMLElement>(
	...
	useEffect(() => {
		if (!ref) return;
		// Use assignRef helper to safely support function or object refs without mutating the incoming ref param
		assignRef<T | null>(ref as React.Ref<T | null>, domRef.current);
	}, [ref]);
